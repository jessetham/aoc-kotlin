...
fun main() {
    ...
    val (validTickets, invalidSum) = check(input)
    // Part 1
    println(invalidSum)

    val fields = findFields(validTickets)
    val startsWithDeparture = fields.filter { it.key.startsWith("departure") }.map { myTicket[it.value].toLong() }
    // Part 2
