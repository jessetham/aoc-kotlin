...
object Day10 {
    ...
    fun countWaysDpBottomUp(adapters: List<Int>): Long {
        val dp = adapters.associateWith { 0L }.toMutableMap()
        dp[0] = 1
        for (adapter in adapters) {
            for (i in 1..3) {
                if (dp.contains(adapter + i)) {
                    dp[adapter + i] = dp.getValue(adapter + i) + dp.getValue(adapter)
                }
            }
        }
        return dp.getValue(adapters.last())
    }
}
...
fun main() {
    ...
    adapters.sort()
    val joltDifferences = adapters.windowed(2).map { (a, b) -> b - a }

    // Part 1
    println(joltDifferences.count { it == 3 } * joltDifferences.count { it == 1 })

    // Part 2
    println(Day10.countWays(joltDifferences))
    // Alternatively...
    println(Day10.countWaysDp(0, deviceAdapter, adapters.toSet(), mutableMapOf()))
    // Finally...
    println(Day10.countWaysDpBottomUp(adapters))
}